...
public class OPC implements Runnable
{
  ...
  OPC(PApplet parent, String host, int port)
  {
    ...
    thread.start();
    this.enableShowLocations = true; // SET WHETHER TO SEE THE LED DOTS OR NOT
  ...
  void led(int index, int x, int y)  
  {
    ...
    pixelLocations[index] = ((width + (x % width)) % width) + (width * ((height + (y % height)) % height));  // CHANGED TO ALLOW PIXELS TO OVERFLOW THE SCREEN
  }
